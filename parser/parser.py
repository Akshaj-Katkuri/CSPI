...
class Parser:
    ...
    def repeat_expr(self):
        ...
        if self.current_token.matches(TYPE_KEYWORD, "UNTIL"):
            ...
            return result.success(RepeatUntilNode(condition, body, True))
        else:
    ...
    def list_expr(self):
        ...
        if self.current_token.type == TYPE_RSQUARE:
            ...
            self.advance()
            return result.success(
                ListNode([], pos_start=start_pos, pos_end=self.current_token.pos_end)
            )
    ...
    def atom(self):
        ...
        if token.matches(TYPE_KEYWORD, "FALSE"):
            ...
            return result.failure(
                EndOfFile(token.pos_start, token.pos_end, "Reached end of file")
            )
    ...
    def term(self):
        return self.binary_operation(
            self.factor, (TYPE_DIV, TYPE_MUL, (TYPE_KEYWORD, "MOD"))
        )
    ...
    def comp_expr(self):
        ...
        node = result.register(
            self.binary_operation(
                self.arith_expr,
                (TYPE_EE, TYPE_NE, TYPE_LT, TYPE_GT, TYPE_LTE, TYPE_GTE),
            )
        )
    ...
    def expr(self):
        ...
        if (
            self.current_token.type == TYPE_IDENTIFIER
            and self.get_next_token().type == TYPE_EQ
        ):
            var_name = self.current_token
        ...
        node = result.register(
            self.binary_operation(
                self.comp_expr, ((TYPE_KEYWORD, "AND"), (TYPE_KEYWORD, "OR"))
            )
        )
    ...
    def statement(self):
        ...
        if self.current_token.matches(TYPE_KEYWORD, "BREAK"):
            ...
            self.advance()
            return result.success(
                BreakNode(pos_start, self.current_token.pos_end.copy())
            )
    ...
    def statements(self, initial=False):
        ...
        if result.error:
            if isinstance(result.error, EndOfFile) and initial:
                ...
                return result.failure(
                    InvalidSyntaxError(
                        result.error.pos_start,
                        result.error.pos_end,
    ...
    def binary_operation(self, func_a, op_tokens, func_b=None) -> ParseResult:
        ...
        while (
            self.current_token.type in op_tokens
            or (self.current_token.type, self.current_token.value) in op_tokens
        ):
            op_token = self.current_token
            result.register_advancement()
            self.advance()
